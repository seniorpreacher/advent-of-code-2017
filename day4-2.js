const data = `vxjtwn vjnxtw sxibvv mmws wjvtxn icawnd rprh
...
const solve = (data) => data
	.split('\n')
	.map((r) => r.split(' ').map(word => word.split('').sort().join('')))
	.filter((line) => Array.from(new Set(line)).length === line.length, 0)
	.length
